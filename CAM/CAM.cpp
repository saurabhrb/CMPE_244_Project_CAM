...
bool CAM::begin(uint16_t baud)
{
    Uart3::getInstance().init(baud, 10, 10);
    return reset();
}
...
{
    uint8_t args[] = { 0x0C, 0x0, 0x0A, 0, 0, frameptr >> 8, frameptr & 0xFF, 0, 0, 0, n,
    CAMERADELAY >> 8, CAMERADELAY & 0xFF };

    if (!runCommand(VC0706_READ_FBUF, args, sizeof(args), 5, false)) return 0;

    // read into the buffer PACKETLEN!
    if (readResponse(n + 5, CAMERADELAY) == 0) return 0;
...
//        flush_buff();
    ...
    if(cmd == VC0706_GET_FBUF_LEN)
        {
            u0_dbg_printf("VC0706_GET_FBUF_LEN\n");
        }
...
void CAM::sendCommand(uint8_t cmd, uint8_t args[] = 0, uint8_t argn = 0)
{
    ...
    Uart3::getInstance().putChar(serialNum);
    Uart3::getInstance().putChar(cmd);

    for (uint8_t i = 0; i < argn; i++) {
        Uart3::getInstance().putChar(args[i]);
    }
}
