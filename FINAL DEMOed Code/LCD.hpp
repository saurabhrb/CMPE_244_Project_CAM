...
#define RA8875_DARK_ORANGE      0xFB85
...
public:
    LCD()
    {
        curr= {0,0};}
    ~LCD() {};
    LCD(LabGPIO_X cs, LabGPIO_X rst, unsigned int max_clock_mhz= 2);

    void set_ssp_freq(unsigned int max_clock_mhz);
    ...
    void textWriteChar(const char buffer);
    void textWriteOFF(const char* buffer, uint16_t len = 0,uint16_t Xoffset=0, uint16_t Yoffset =0);
    ...
    //source: https://github.com/sumotoy/RA8875/blob/0.70/RA8875.h
    ...
    inline uint16_t htmlTo565(int32_t color_)
    {
    ...
    void drawPixels(uint8_t p[], uint16_t count, int16_t x, int16_t y);
...
*/
    ...
    {
        if (x < 0) curr.x = 0;
        if (x >= _width) curr.x = _width-1;
        writeReg(RA8875_CURH0, curr.x & 0xFF);
        writeReg(RA8875_CURH0+1, (curr.x >> 8));
    }
    ...
    void setY(int16_t y)
    {
        ...
        if (y >= _height) y = _height - 1;
        writeReg(RA8875_CURV0, curr.y & 0xFF);
        writeReg(RA8875_CURV0 + 1, curr.y >> 8);
...
private:
    ...
    uint8_t _textScale;
    unsigned int max_SPI_clock_mhz;
