...
void OV7670::wait_us(uint8_t us)
{
    uint64_t now = sys_get_uptime_us() + us;
    while(now<=sys_get_uptime_us()){
//        vTaskDelay(0);
        ;
    }
}
...
//    _i2c.write(data);
//    _i2c.stop();
    _i2c->writeReg(Dev_addr,addr,data);
}
...
//    _i2c.stop();
    data = _i2c->readReg(Dev_addr,addr);
